...
struct Args {
    ...
    /// Source URI included in the JobSpec returned to agents.
    #[arg(long, env = "MX8_SOURCE_URI")]
    source_uri: Option<String>,

    /// Sink URI included in the JobSpec returned to agents.
    #[arg(long, env = "MX8_SINK_URI")]
    sink_uri: Option<String>,

    /// AWS region included in the JobSpec returned to agents.
    #[arg(long, env = "MX8_AWS_REGION")]
...
impl ConfiguredJobSpec {
    fn build(&self, job_id: &str) -> CoreJobSpec {
        CoreJobSpec {
...
fn parse_configured_job_spec(args: &Args) -> anyhow::Result<Option<ConfiguredJobSpec>> {
    let any_set = args.source_uri.is_some()
        || args.sink_uri.is_some()
        || args.aws_region.is_some()
        || args.transforms_json.is_some();
    if !any_set {
        return Ok(None);
    }
    ...
    let source_uri = args
        .source_uri
        .clone()
        .ok_or_else(|| anyhow::anyhow!("MX8_SOURCE_URI is required when configuring a JobSpec"))?;
    let sink_uri = args
        .sink_uri
        .clone()
        .ok_or_else(|| anyhow::anyhow!("MX8_SINK_URI is required when configuring a JobSpec"))?;
    let aws_region = args
        .aws_region
        .clone()
        .ok_or_else(|| anyhow::anyhow!("MX8_AWS_REGION is required when configuring a JobSpec"))?;
    let transforms_json = args.transforms_json.clone().ok_or_else(|| {
        anyhow::anyhow!("MX8_TRANSFORMS_JSON is required when configuring a JobSpec")
    })?;
...
impl Coordinator for CoordinatorSvc {
    ...
    async fn register_node(
        ...
        let resp = RegisterNodeResponse {
            ...
            job_spec: JOB_SPEC_TEMPLATE
                .get()
                .and_then(|template| template.as_ref())
                .map(|template| template.build(&req.job_id).to_wire()),
        };
...
async fn main() -> Result<()> {
    ...
    async move {
        ...
        let configured_job_spec = parse_configured_job_spec(&args)?;
        if JOB_SPEC_TEMPLATE.set(configured_job_spec.clone()).is_err() {
            tracing::warn!("job spec template already initialized; keeping existing global value");
        }
        if let Some(spec) = configured_job_spec.as_ref() {
            info!(
